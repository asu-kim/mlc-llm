...
class RagChatModel(private val context: Context) {
    ...
    private var userTimezone: String = "Unknown"
    ...
    private fun getEventsForQuery(
        ...
        return when {
            ...
            "next week" in normalized -> {
                val nextWeekStart =
                    now.plusWeeks(1).with(java.time.DayOfWeek.MONDAY).toLocalDate()
                ...
                eventsWithTime.filter {
                    ...
                    eventDate != null &&
                            !eventDate.isBefore(nextWeekStart) &&
                            !eventDate.isAfter(nextWeekEnd)
                }
            }
    ...
    private fun loadVecFromKGProvider(context: Context): List<EmbeddedText> {
        ...
        val uri =
            Uri.parse("content://com.example.knowledgegraph.kgprovider/knowledge_graph_vec")
                ...
                lines.forEachIndexed { index, line ->
                    ...
                    try {
                        ...
                        if (embeddingList.isNotEmpty()) {
                            ...
                            val norm =
                                sqrt(embeddingList.fold(0f) { acc, x -> acc + x * x })
                        ...
                        Log.e(
                            "RAG_PARSE",
                            "Failed to parse embedding at line $index: ${e.message}"
                        )
                    }
                }
            }
    ...
    private fun loadUserProfileFromKGProvider() {

        val uri =
            Uri.parse("content://com.example.knowledgegraph.kgprovider/knowledge_graph")
        ...
        try {
                    ...
                    lines.forEach { line ->
                        ...
                        if (parts.size >= 3 && parts[0] == "user") {
                            ...
                            when (parts[1]) {
                                ...
                                "first_name" -> userName = parts[2]

                                "role" -> userRole = parts[2]

                                "timezone" -> userTimezone = parts[2]

                                "located_at" -> userLocation = parts[2]
                            }
                        }
                    }
                }
            ...
            Log.d(
                "RAG_PROFILE",
                "Loaded profile: $userName, $userRole, $userTimezone, $userLocation"
            )
    ...
    private fun retrieveTopK(
        ...
        fun phraseOverlapScore(query: String, text: String): Float {
            ...
            val queryTokens =
                query.split(" ").filter { it.length > 2 }.toSet()

            val textTokens =
                normalizeText(text).split(" ").toSet()
        ...
        fun temporalProximityScore(text: String): Float {
            ...
            return if (eventTime != null) {
                ...
                val hoursUntil =
                    Duration.between(now, eventTime).toHours().toFloat()
        ...
        val scoredData = data.map {
            ...
            val cosine =
                cosineSimilarity(queryVec, it.embedding)

            val overlap =
                phraseOverlapScore(normalizedQuery, it.text)

            val temporal =
                temporalProximityScore(it.text)
        ...
        val keywordMatches = data.filter {
            ...
            normalizedQuery.split(" ").any { word ->
                word.length > 3 &&
                        it.text.lowercase().contains(word.lowercase())
            }
        }
